    ...
    <body>
        ...
        <div class="container">
            <div class="control-panel">
                ...
                <div class="control-row" style="border-top: 1px solid #d2d2d7; padding-top: 1rem; margin-top: 1rem;">
                    <strong>⚠️ Reset:</strong>
                    <button class="btn btn-delete" onclick="resetAll()" id="btn-reset">🗑️ Clear All Data & Start Fresh</button>
                    <span style="font-size: 0.75rem; color: #86868b;">(Deletes scan results and embeddings)</span>
                </div>
        ...
        <script>
            ...
            async function resetAll() {
                if (!confirm('⚠️ This will delete all scan results, embeddings, and feedback. Are you sure?')) {
                    return;
                }
                ...
                try {
                    ...
                    if (response.ok) {
                        showStatus('success', '✓ All data cleared! Ready to start fresh.');
                        ...
                        document.getElementById('scan-limit').value = '20';
                    } else {
                        const error = await response.json();
                        showStatus('error', error.detail || 'Reset failed');
                    }
                } catch (error) {
...
async def reset_all():
    ...
    global SIMILAR_GROUPS, SCAN_DATA, EMBEDDING_STORE, CURRENT_THRESHOLD
    ...
    try:
        ...
        if scan_file.exists():
            scan_file.unlink()
            logger.info("Deleted scan results")
