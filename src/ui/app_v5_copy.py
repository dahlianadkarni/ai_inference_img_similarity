"""FastAPI application for duplicate photo review with tab-based workflow (v5).

Based on v3 (working version) with three v4 features added:
1. Tab-based layout - separate Scanner and AI panels
2. Clear Scan button + /api/clear-scan endpoint
3. Separate status elements per tab (processing-status-scan, processing-status-ai)
"""
        ...
        <script>
            ...
            async function clearScanResults() {
                ...
                try {
                    ...
                    document.getElementById('scanner-results').innerHTML = '<div class="empty-state"><h3>📊 No scanner results yet</h3><p>Click "Start Scan" above to find exact and perceptual duplicates.</p></div>';
            ...
            async function clearEmbeddings() {
                ...
                try {
                    ...
                    document.getElementById('ai-results').innerHTML = '<div class="empty-state"><h3>🧠 No AI results yet</h3><p>Run "Generate Embeddings" after scanning to find AI-detected similar groups.</p></div>';
            ...
            function render() {
                ...
                if (!hasScanData) {
                    scanHtml = '<div class="empty-state"><h3>📊 No scanner results yet</h3><p>Click "Start Scan" above to find exact and perceptual duplicates.</p></div>';
                } else {
                    ...
                    scanHtml += '<h3 style="margin: 0 0 0.5rem 0; color: #007aff;">ℹ️ About AI Embeddings</h3>';
                ...
                if (!hasAIData) {
                    aiHtml = '<div class="empty-state"><h3>🧠 No AI results yet</h3><p>Run "Generate Embeddings" after scanning to find AI-detected similar groups.</p></div>';
                } else {
                    // New AI Discoveries
                    aiHtml += renderUnifiedSection({
                        ...
                        type: 'ai',
                        tipMessage: '💡 These AI groups contain images <strong>not found in exact or perceptual duplicates</strong>. These are truly new discoveries based on visual content analysis.',
                    ...
                    aiHtml += renderUnifiedSection({
                        ...
                        type: 'ai',
                        tipMessage: '💡 These AI groups contain images that are also in <strong>Exact Duplicate</strong> groups.',
                        tipBackground: '#f5f5f7',
                        tipTextColor: '#1d1d1f',
                        buttonColor: '#34c759',
                    ...
                    aiHtml += renderUnifiedSection({
                        ...
                        type: 'ai',
                        tipMessage: '💡 These AI groups contain images that are also in <strong>Perceptual Duplicate</strong> groups.',
                        tipBackground: '#f5f5f7',
                        tipTextColor: '#1d1d1f',
                        buttonColor: '#ff9500',
                    ...
                    aiHtml += '<h3 style="margin: 0 0 0.5rem 0; color: #007aff;">📊 AI Analysis Summary</h3>';
...
async def run_scan(request: ScanRequest):
    ...
    if SERVER_MODE == "demo":
        global DEMO_PROCESSING_STATUS
        status = DEMO_PROCESSING_STATUS
        scan_output = "scan_results_demo.json"
        dup_output = "scan_duplicates_demo.json"
        cache_file = ".cache_demo/scan_cache.json"
    else:
        global PROCESSING_STATUS
        status = PROCESSING_STATUS
        scan_output = "scan_for_embeddings.json"
        dup_output = "scan_duplicates.json"
...
async def batch_delete(group_type: str):
    ...
    if SERVER_MODE == "demo":
        scan_path = PROJECT_ROOT / "scan_results_demo.json"
        duplicates_path = PROJECT_ROOT / "scan_duplicates_demo.json"
    else:
        scan_path = PROJECT_ROOT / "scan_for_embeddings.json"
